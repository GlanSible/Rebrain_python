...
def host_extended_info():
    # Define host_extended_info dict as data structure
    host_extended_info = {}
    ...
    host_extended_info['host_information'] = host_information
    ...
    for snicaddr in psutil.net_if_addrs():
        if not snicaddr.startswith(tuple(addr_templates)):
            ...
            pass
        host_extended_info['network'] = [iface_dict, ip_dict]
    ...
    for sdiskpart in psutil.disk_partitions():
        if not str(sdiskpart.device).startswith('/dev/loop'):
            ...
            host_extended_info['disk'] = disk_list

    logging.info("Gathering RAM information:")
    host_extended_info['memory'] = psutil.virtual_memory()._asdict()
    # Iterate over memory dict to transform bytes into MiB
    for k, v in host_extended_info['memory'].items():
        if v <= 100:
            pass
        else:
            v = round(v / 1024 ** 2)
            host_extended_info['memory'][k] = v
    ...
    host_extended_info['cpu'] = cpu

    logging.info("Gathering LA information:")
    load_average = dict(zip(['1 min', '5 min', '15 min'], psutil.getloadavg()))
    host_extended_info['load_average'] = load_average
    # Round float values up to 2 signs
    for k, v in host_extended_info['load_average'].items():
        v = round(v, 2)
        host_extended_info['load_average'][k] = v

    logging.info("Serializing gathered data.")
    jsoned_host_extended_info = json.dumps(host_extended_info, indent=True)

    logging.info(f"""Gathered info about {host_extended_info['host_information']['name']} server:
                 {jsoned_host_extended_info}""")

    loaded_host_extended_info = json.loads(jsoned_host_extended_info)

    return loaded_host_extended_info


try:
    r = requests.post(
        'http://127.0.0.1:8000/api/servers/add-extended',
        json=host_extended_info())
